...
from .util import _num_samples, clone, ConvergenceWarning

from sklearn.gaussian_process.kernels import Kernel as GPK
...
# +
class DynamicsModelKernel(StationaryKernelMixin, NormalizedKernelMixin, Kernel, GPK):
    def __init__(self, result_path, init_model_ind, length_scale=1.0, length_scale_bounds=(1e-5, 1e5)):
        self.length_scale = length_scale
        self.length_scale_bounds = length_scale_bounds
        self.result_path = result_path
        self.init_model_ind = init_model_ind   #To fill the gap between model index and model dist list index
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        length_scale = _check_length_scale(X, self.length_scale)
        selected_dists = []
        with open(self.result_path+"/model_distances", "r") as fp:
            model_dists = json.load(fp)
        dists_matrix = np.array(model_dists)

        if Y is None:
            for i in range(len(X)):
                for j in range(len(X)):
                    if j > i:
                        curr_ind = int(X[i][0]) - self.init_model_ind
                        iter_ind = int(X[j][0]) - self.init_model_ind
                        selected_dists.append(dists_matrix[curr_ind][iter_ind])
            selected_dists = np.array(selected_dists)
            K = np.exp(-0.5 * selected_dists)
            K = squareform(K)
            np.fill_diagonal(K, 1)
            ...
            for i in range(len(X)):
                ...
                for j in range(len(Y)):
                    X_ind = int(X[i][0]) - self.init_model_ind
                    Y_ind = int(Y[j][0]) - self.init_model_ind
                    iter_dist.append(dists_matrix[X_ind][Y_ind])
                selected_dists.append(iter_dist)
            selected_dists = np.array(selected_dists)
            K = np.exp(-0.5 * selected_dists)
        ...
        if eval_gradient:
            ...
            if not self.anisotropic or length_scale.shape[0] == 1:
                K_gradient = (K * squareform(selected_dists))[:, :, np.newaxis]
                return K, K_gradient
            elif self.anisotropic:
                # We need to recompute the pairwise dimension-wise distances
                K_gradient = (X[:, np.newaxis, :] - X[np.newaxis, :, :]) ** 2 / (
