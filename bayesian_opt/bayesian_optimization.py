...
import numpy as np
import pandas as pd
from bayesian_opt.constraint import ConstraintModel

from .target_space import TargetSpace
from .event import Events, DEFAULT_EVENTS
from .logger import _get_default_logger
from .util import UtilityFunction, acq_max, ensure_rng
from .kernels import Matern, DynamicsModelKernel
...
class BayesianOptimization(Observable):
    ...
    def __init__(self,
        ...
        self._gp = GaussianProcessRegressor(
            kernel=DynamicsModelKernel(result_path, init_model_ind),
            alpha=1,
            normalize_y=True,
    ...
    def random_points(self, n_point):
        selected_points = []
        
        while len(selected_points) != n_point:
            suggestions = self._space.random_sample(self.init_model_ind, self.last_model_ind+1)
            selected_points.append(suggestions[0])
    ...
    def suggest(self, utility_function):
        ...
        if len(self._space) == 0:
            suggestion = self._space.random_sample(self.init_model_ind, self.last_model_ind+1)
    ...
    def _prime_queue(self, init_points):
        ...
        for _ in range(init_points):
            self._queue.add(self._space.random_sample(self.init_model_ind, self.last_model_ind+1))
