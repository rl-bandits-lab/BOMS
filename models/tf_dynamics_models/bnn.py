...
class BNN:
    ...
    def __init__(self, params):
        ...
        self._observation_shape = params['obs_dim']
        self._action_shape = params['act_dim']
    ...
    def train(self, inputs, targets, load_model, result_path,
        ...
        # print('---variable:', ([layer.get_model_vars(0, self.sess) for layer in self.var_layers]))
        t0 = time.time()
        grad_updates = 0
        for epoch in epoch_iter:
            ...
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    ...
                    if not load_model:   # save every model in training process 
                        self.save(savedir=result_path+'/Trained_Dynamics_Model', type_name=str(epoch))
        ...
        return OrderedDict(model_metrics), epoch+1
    ...
    def save(self, savedir, type_name):
        ...
        if not self.separate_mean_var:
            with open(os.path.join(model_dir, '{}_{}.nns'.format(self.name, type_name)), "w+") as f:
            ...
            with open(os.path.join(model_dir, '{}_{}.nns'.format(self.name, type_name)), "w+") as f:
                for layer in self.layers:
                    f.write("%s\n" % repr(layer))
            with open(os.path.join(model_dir, '{}_{}_var.nns'.format(self.name, type_name)), "w+") as f:
        ...
        savemat(os.path.join(model_dir, '{}_{}.mat'.format(self.name, type_name)), var_vals)
